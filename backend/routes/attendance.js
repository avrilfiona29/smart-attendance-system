  ...
  py.on("close", (code) => {
    ...
    try {
      ...
      attendance.push({
        ...
        groupImage: path.relative(path.join(__dirname, ".."), imagePath),
...
router.get('/history/:date', (req, res) => {
  ...
  try {
    ...
    if (filtered.length === 0) return res.json({ message: "No attendance found for this date" });
    res.json({ date: requestedDate, records: filtered });
  } catch (err) {
